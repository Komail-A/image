...
class ImageResponse(BaseModel):
    ...
    image_id: str
    filename: str
    s3_url: str  # Direct S3 URL (requires auth)
    presigned_url: str  # Presigned URL (public, temporary)
    presigned_expires_in: int  # Seconds until presigned URL expires
    bucket: str
    prompt: str
...
async def startup():
    ...
    print(f"Presigned URLs enabled (expire in {PRESIGNED_URL_EXPIRY}s)")
    print("=" * 60 + "\n")
...
async def generate(req: ImageRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        print(f"  ✓ Direct URL: {s3_url}")
        print(f"  ✓ Presigned URL: {presigned_url[:80]}...")
        print(f"  ✓ Expires in: {PRESIGNED_URL_EXPIRY}s ({PRESIGNED_URL_EXPIRY // 86400} days)\n")
        ...
        return ImageResponse(
            ...
            presigned_expires_in=PRESIGNED_URL_EXPIRY,
            bucket=S3_BUCKET,
            prompt=req.prompt,
            lora_used=lora_used,
            seed=seed
